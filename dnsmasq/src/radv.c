...
static int deprecate_ra(char *int_interface, int count)
{
    ...
    deprecate_old_prefix(g_last_prefix, int_interface);
...
int set_mode_of_ra(const char *mode)
{
    ...
    if (!strcmp(cmd, "true")) {
        ...
        active_old_prefix(g_last_prefix, g_int_interface);
        block_old_prefix_list(g_int_interface);
        print_old_prefix_list(g_int_interface);
