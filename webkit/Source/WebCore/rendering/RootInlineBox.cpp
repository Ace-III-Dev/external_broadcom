...
RootInlineBox::RootInlineBox(RenderBlock* block)
    ...
    , m_lineHeightAdjust(-1)
    , m_thaiDescentAdjust(-1)
...
//SAMSUNG CHANGES : MPSG100006193 - use bTextSelection condition >>
    if ((bTextSelection || rootbox ->selectionState() != RenderObject::SelectionNone) 
	    && renderer()->document() && renderer()->document()->settings() 
		&& renderer()->document()->settings()->editableSupportEnabled() == false)
//SAMSUNG CHANGES <<
    {
...
void RootInlineBox::ascentAndDescentForBox(InlineBox* box, GlyphOverflowAndFallbackFontsMap& textBoxDataMap, int& ascent, int& descent,
                                           bool& affectsAscent, bool& affectsDescent)
//SISO HTMLComposer End										   
{
...
//SISO_HTMLComposer start
            if(box->renderer()->document() && box->renderer()->document()->settings() 
                && box->renderer()->document()->settings()->editableSupportEnabled()) {
                m_lineHeightAdjust = 0;
                if(box->renderer()->style()) {
                    int tempDescent = box->renderer()->style()->fontMetrics().descent();
                    int delta = (tempDescent>0)?(tempDescent/2):2;
                    ascent += delta ;
                    m_lineHeightAdjust += delta;
                }
            }
//SISO_HTMLComposer end
...
//SISO_HTMLComposer start
	if(box->renderer()->document() && box->renderer()->document()->settings() 
		&& box->renderer()->document()->settings()->editableSupportEnabled()) {
		int delta = (descent>0)?(descent/2):2;
		ascent += delta;
		m_lineHeightAdjust = 0;
                InlineBox* leaf = firstLeafChild();
                if(leaf && leaf->isInlineTextBox() && !leaf->isLineBreak())
                    m_lineHeightAdjust += delta;
		affectsAscent = true;
		if(isThaiReshapeChars()){
			descent += delta;
			m_lineHeightAdjust += delta;
			affectsDescent = true;
		}
...
//SISO_HTMLComposer start
bool RootInlineBox::isThaiReshapeChars() {

	if(thaiDescentAdjust() == -1) {
		setThaiDescentAdjust(0);
		InlineBox* leaf = firstLeafChild();
		for (; leaf; leaf = leaf->nextLeafChild()) {
			if(leaf->isInlineTextBox()){
				InlineTextBox* textChild = static_cast<InlineTextBox*>(leaf);
				const UChar* characters = 0x00;
				if(textChild->textRenderer() && textChild->textRenderer()->text()) {
					characters = textChild->textRenderer()->text()->characters() + textChild->start();
				}
				if(characters){
					String str = String(characters);
					if(str.length()){
						for(int i=0; i<(str.length()-1); i++){  
							if(((int)characters[i] == THAI_DO_CHADA || (int)characters[i] == THAI_TO_PATAK)
								&& ((int)characters[i+1] == THAI_SARA_U || (int)characters[i+1] == THAI_SARA_UU)) {
								setThaiDescentAdjust(1);
								break;
							} 
						}
					}
