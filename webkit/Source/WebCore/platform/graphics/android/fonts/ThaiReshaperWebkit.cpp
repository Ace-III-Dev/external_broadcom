...
int isThaiReshaperCharhw(int32_t c) {
	...
	} else if ((c >= MAI_EK) && (c <= MAI_CHATTAWA)) {
		return THAIRESHAPESARAAM;
	} else {
...
int isThaiReshaperTonehw(int32_t uniCode, int32_t consonantType) {
	if (uniCode == SARA_U || uniCode == SARA_UU || uniCode == PHINTHU) {
		if (consonantType == THAIRESHAPEREMOVETAIL)
			...
			return THAIRESHAPEPULLDOWN;
	} else if ((consonantType == THAIRESHAPESHIFTLEFT) && (((uniCode >= MAI_EK ) && (uniCode <= NIKHAHIT)) || ((uniCode
			>= SARA_AM) && (uniCode <= SARA_Uee)) || (uniCode == MAI_HAN_AKAT)
			|| (uniCode == MAI_TAI_KHU))) {
		return THAIRESHAPESHIFTLEFT;
	} else if ((consonantType == THAIRESHAPESARAAM) && (uniCode == SARA_AM)) {
		return THAIRESHAPESARAAM;
	} else {
		return NOTTHAIRESHAPE;
	}
        ...
        if(isThai(string[i])){
            ...
            switch(cT){
                ...
                case THAIRESHAPESHIFTLEFT: 
                    ...
                    break;
                 case THAIRESHAPESARAAM:
                    result=isThaiReshaperTonehw(string[i+1],cT);
                    if(equal(result, THAIRESHAPESARAAM)){
                        if (equal(string[i], MAI_EK)) {
                            string[i++] = MAI_EK_SARA_AM;
                             string[i++] = SARA_AM_FIRST;
                        } else if (equal(string[i], MAI_THO)) {
                            string[i++] = MAI_THO_SARA_AM;
                             string[i++] = SARA_AM_FIRST;
                        } else if (equal(string[i], MAI_TRI)) {
                            string[i++] = MAI_TRI_SARA_AM;
                             string[i++] = SARA_AM_FIRST;
                        } else {
                            string[i++] = MAI_CHATTAWA_SARA_AM;
                             string[i++] = SARA_AM_FIRST;
                        }
                        replaced = true;
                    }
